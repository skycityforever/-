...
class EnhancedModelTrainer:
    def __init__(self, data_paths, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        # 新增：UserCF模型相关属性
        self.user_cf_model = None  # 加载的UserCF模型
        self.user_cf_user_idx = None  # UserCF用户索引
        self.user_cf_book_idx = None  # UserCF书籍索引
        self.user_cf_book_list = None  # UserCF书籍列表
        self.user_cf_top_k = None  # UserCF近邻数量

        # BERT模型相关属性
        self.bert_model = None
        self.bert_user_encoder = None
        self.bert_book_encoder = None
        self.bert_seq_len = 5
        self.bert_max_len = 10
    ...
    def load_and_preprocess_data(self, test_size=0.1, random_state=42, train_word2vec=True):
        ...
        merged_books_encoded['二级分类_encoded'] = merged_books_encoded['二级分类'].astype(
    ...
    def train_hybrid(self, params):
        ...
        class HybridModel:
            def __init__(self, cf_model, lstm_model, user_encoder, book_encoder,
                ...
                self.user_idx = cf_model.user_idx
                self.book_idx = cf_model.book_idx
                self.book_list = cf_model.book_list

            def predict(self, user_id, user_idx=None, book_idx=None, book_list=None, user_inter=None):
                ...
                if user_inter is not None and len(user_inter) >= self.seq_len:
                    lstm_rec, _ = self.trainer._lstm_predict(
    ...
    @staticmethod
    def calculate_precision_recall(model, valid_data, user_list=None, book_list=None):
        """原有评估逻辑（保持不变）"""
        hits = 0
        total_pred = 0
        total_true = 0

        user_idx = {u: i for i, u in enumerate(user_list)} if user_list else model.user_idx
        book_idx = {b: i for i, b in enumerate(book_list)} if book_list else model.book_idx
    ...
    def _save_best_model(self, model, model_type, epoch, f1_score, **kwargs):
        ...
        save_data.update(kwargs)  # 包含title_vec_dim、user_encoder等（如果存在）
        ...
        if isinstance(model, torch.nn.Module):
            ...
            # 保存UserCF或混合模型的参数
            save_data.update({
                "user_similarity": getattr(model, 'user_similarity', None),
                "user_item_matrix": getattr(model, 'user_item_matrix', None),
                "user_idx": getattr(model, 'user_idx', None),
                "book_idx": getattr(model, 'book_idx', None),
                "book_list": getattr(model, 'book_list', None),
                "top_k": getattr(model, 'top_k', None),
                # 混合模型额外参数
                "cf_weight": getattr(model, 'cf_weight', None),
                "lstm_weight": getattr(model, 'lstm_weight', None),
    ...
    def load_lstm_model(self, model_path):
        ...
        title_vector_dim = checkpoint.get("title_vector_dim", 0)  # 恢复书名向量维度
    ...
    def predict_lstm_test(self, submission_path="submission_lstm.csv", confidence_threshold=0.5):
        ...
        if len(pred_list) > 0 and len(true_list) > 0:
            precision, recall = self.calculate_precision_recall(None, pd.DataFrame(
                {'user_id': range(len(pred_list)), 'book_id': true_list}),
                                                                pred_list, true_list)
    ...
    # -------------------------- UserCF模型加载和预测 --------------------------
    def load_user_cf_model(self, model_path):
        """新增：加载UserCF模型"""
        if not os.path.exists(model_path):
        ...
        # 恢复UserCF核心参数
        user_similarity = checkpoint.get("user_similarity")
        user_item_matrix = checkpoint.get("user_item_matrix")
        user_idx = checkpoint.get("user_idx")
        book_idx = checkpoint.get("book_idx")
        book_list = checkpoint.get("book_list")
        top_k = checkpoint.get("top_k", 20)

        # 验证必要参数
        required_params = [user_similarity, user_item_matrix, user_idx, book_idx, book_list]
        if any(param is None for param in required_params):
            raise ValueError("UserCF模型文件缺少必要参数")

        # 重建UserCF模型
        self.user_cf_model = UserCF(top_k=top_k)
        self.user_cf_model.user_similarity = user_similarity
        self.user_cf_model.user_item_matrix = user_item_matrix
        self.user_cf_model.user_idx = user_idx
        self.user_cf_model.book_idx = book_idx
        self.user_cf_model.book_list = book_list
        self.user_cf_model.top_k = top_k

        # 保存模型参数到实例变量
        self.user_cf_user_idx = user_idx
        self.user_cf_book_idx = book_idx
        self.user_cf_book_list = book_list
        self.user_cf_top_k = top_k

        self.logger.info(f"UserCF模型加载成功: {model_path}，近邻数量: {top_k}")
        print(f"UserCF模型加载成功，包含 {len(user_idx)} 个用户，{len(book_list)} 本书籍")

    def predict_user_cf_test(self, submission_path="submission_user_cf.csv"):
        """新增：UserCF模型预测测试集"""
        if self.user_cf_model is None or self.test_data is None:
            raise ValueError("请先加载UserCF模型和测试数据（调用load_user_cf_model和load_and_preprocess_data）")

        submission_data = []
        pred_list = []
        true_list = []
        unknown_user_count = 0  # 统计未知用户数量

        # 按用户分组并按时间排序
        grouped = self.test_data.groupby("user_id")
        for user_id, group in tqdm(grouped, desc="UserCF预测测试集用户"):
            sorted_group = group.sort_values("借阅时间").reset_index(drop=True)
            # 获取用户最后一次借阅的书籍（作为真实标签）
            user_last_book = sorted_group["book_id"].iloc[-1] if not sorted_group.empty else None

            # 预测
            if user_id not in self.user_cf_user_idx:
                # 未知用户：推荐最后一次借阅的书籍
                pred_book = user_last_book
                unknown_user_count += 1
            else:
                # 已知用户：使用UserCF预测
                pred_book = self.user_cf_model.predict(
                    user_id=user_id,
                    user_idx=self.user_cf_user_idx,
                    book_idx=self.user_cf_book_idx,
                    book_list=self.user_cf_book_list
                )

            # 收集结果
            if pred_book is not None and user_last_book is not None:
                submission_data.append({"user_id": user_id, "book_id": pred_book})
                pred_list.append(pred_book)
                true_list.append(user_last_book)

        # 生成提交文件
        submission_df = pd.DataFrame(submission_data)
        submission_df.to_csv(submission_path, index=False, encoding="utf-8")
        self.logger.info(f"UserCF提交文件已保存至: {submission_path}，共 {len(submission_df)} 条记录")
        self.logger.info(f"未知用户数量: {unknown_user_count} / {len(grouped)} 个用户")
        print(f"提交文件已保存至: {submission_path}，共 {len(submission_df)} 条记录")
        print(f"未知用户数量: {unknown_user_count} / {len(grouped)} 个用户")

        # 计算评估指标
        if len(pred_list) > 0 and len(true_list) > 0:
            precision, recall = self.calculate_precision_recall(
                self.user_cf_model,
                pd.DataFrame({'user_id': range(len(pred_list)), 'book_id': true_list}),
                self.user_cf_book_list,
                self.user_cf_book_list
            )
            f1 = calculate_f1(precision, recall)
            self.logger.info(f"UserCF测试集评估指标 - 精确率: {precision:.4f}, 召回率: {recall:.4f}, F1: {f1:.4f}")
            print(f"UserCF测试集评估指标 - 精确率: {precision:.4f}, 召回率: {recall:.4f}, F1: {f1:.4f}")
        else:
            self.logger.warning("无有效预测结果，无法计算评估指标")
            print("无有效预测结果，无法计算评估指标")
    ...
    def predict_bert_test(self, submission_path="submission_bert.csv"):
        ...
        print(f"BERT提交文件已保存至: {submission_path}，共 {len(submission_df)} 条记录")
        print(f"预测失败并使用默认推荐的次数: {failed_count}")
    ...
    def cross_validate(self, model_type, params):
        ...
        for fold, (train_idx, valid_idx) in enumerate(kf.split(self.train_data)):
            ...
            if model_type == 'user_cf':
                ...
                _, _, _, f1 = self.train_bert(params)
...
请输入操作编号(1-7): """)
    ...
    if action == "1":
        # 训练UserCF模型
        trainer.train("user_cf")
    elif action == "2":
        # 加载UserCF模型并预测
        default_cf_path = "model/saved_models/user_cf_best_epoch1_f10.xxxx.pth"  # 请替换为实际的模型路径
        model_path = input(f"请输入UserCF模型路径（默认: {default_cf_path}）: ") or default_cf_path
        try:
            trainer.load_user_cf_model(model_path)
            trainer.predict_user_cf_test()
        except Exception as e:
            print(f"UserCF预测失败: {str(e)}")
    elif action == "3":
        # 训练LSTM模型
        trainer.train("lstm")
    elif action == "4":
        ...
        # 加载BERT模型并预测
        default_bert_path = "model/saved_models/bert_best_epoch10_f10.0085.pth"
        model_path = input(f"请输入BERT模型路径（默认: {default_bert_path}）: ") or default_bert_path
        try:
            trainer.load_bert_model(model_path)
            trainer.predict_bert_test()
        except Exception as e:
            print(f"BERT预测失败: {str(e)}")
    elif action == "7":
        # 训练混合模型
        trainer.train("hybrid")
    else:
        print("无效操作，请输入1-7")
